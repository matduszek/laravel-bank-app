<x-guest-layout>
    <x-auth-card>
        ...
        <h2 class="text-4xl font-bold text-center">Wpłata</h2>
        <br>
        ...
        <form method="POST" action=" {{ route('transaction.post', ['account_id' => $account->id_account]) }}">
            ...
            <div>
                ...
                <x-text-input id="title" class="block mt-1 w-full" type="hidden" name="title" value="Wpłata na lokatę {{ date('d-m-Y') }}" readonly required autofocus />
            ...
            <div>
                ...
                <x-text-input id="amount" class="block mt-1 w-full" type="text" placeholder="format: 00.00" pattern="^[0-9]+(\.[0-9]{1,2})?$" name="amount" value=""  required autofocus />
                <x-input-error :messages="$errors->get('amount')" class="mt-2" />
            </div>

            <!-- Numer kontana jaki przelewamy -->
            <div>
                <x-input-label for="tonumberaccount" :value="__('')" />
                <x-text-input id="tonumberaccount" class="block mt-1 w-full" type="hidden" name="tonumberaccount" value="{{$special_number}}" readonly required autofocus />
                <x-input-error :messages="$errors->get('tonumberaccount')" class="mt-2" />
            </div>
            <br>

            <input type="hidden" name="t_id" value="{{$account->id_account}}">
            <input type="hidden" name="INVES" value="INVES">
