<x-guest-layout>
    ...
    <div class="text-center text-green-500 text-3xl">Kwota przelewu: +{{$amount}} {{$a_c}}</div>
    <div class="text-center text-3xl">Saldo przed transakcją: {{$sal_b}} {{$sal_a_c}}</div>
    <div class="text-center mb-6 text-3xl">Saldo po transakcji: {{$sal_a}} {{$sal_a_c}}</div>
    <div class="text-center mb-6 text-3xl">Przewalutowano po kursie: {{ $curex }}</div>
    <div class="text-center text-3xl">Data transakcji: {{$time}}</div>
    <div class="text-center text-xl">Typ transakcji: PRZYCHODZĄCA</div>
